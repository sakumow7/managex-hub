"""Reusable fictional scenarios for local evaluation and isolated portfolio demos."""
...
def seed_scenarios(db, accounts, workspace_id=None):
    samples = [
        (
    ...
    for index, (title, category, priority, status, location, days, kind, team, agent) in enumerate(samples):
        ...
        db.add(
            AuditEvent(
                ...
                actor_id=accounts["supervisor" if workspace_id else "administrator"].id,
                action="seed_demo" if workspace_id else "seed_it",
...
def seed():
    ...
    with SessionLocal() as db:
        accounts = {}
        for email, name, role, team in ACCOUNTS:
